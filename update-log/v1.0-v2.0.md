...
```
- courseware\_ai\_summary 表扩展 summary 字段为 LONGTEXT，并新增 searchable\_text 和 search\_summary 字段以支持内容检索
- knowledge\_practice\_question 表新增 submitted\_at 字段记录首次提交时间
- 新建 live\_record\_ai\_analysis 表存储直播录播字幕与 AI 总结，并为 recording\_history 表添加索引优化增量检测
- 为 question\_student\_attachment、paper\_student\_attachment、homework\_student\_attachment 表添加 annotated\_url 字段存储教师批注地址
- 修改 course 表 term\_id 字段为非空约束
