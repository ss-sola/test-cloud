...
```
- courseware\_ai\_summary 表：扩展 summary 字段为 LONGTEXT，新增 searchable\_text 和 search\_summary 字段
- knowledge\_practice\_question 表：新增 submitted\_at 字段记录自主练习首次提交时刻
- 创建 live\_record\_ai\_analysis 表用于直播活动录播字幕与 AI 总结
- recording\_history 表：新增索引 idx\_recording\_history\_ai\_created
- question\_student\_attachment、paper\_student\_attachment、homework\_student\_attachment 表：新增 annotated\_url 字段

## 各服务迁移环境变量
```bash
# 无环境变量差异
```
- 更新环境配置文件 \.version/custom\.env

## 更新内容(测试人员)
- refactor: 移除发布自动化模块中 Git tag、repository、ref、SHA 及 Job ID 的本地格式校验
- feat: 接通发布自动化AI生成Markdown并支持相邻tag自动解析
