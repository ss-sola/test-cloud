...
Job ID 由规范化后的 `Idempotency-Key` SHA-256 派生，不使用用户可注入路径；写操作 gate 要求 key 至少 16 个字符。payload hash 对稳定排序 JSON 计算。重复请求在入队前和串行 admission gate 内各检查一次，处理并发竞态；hash 不同返回冲突。
