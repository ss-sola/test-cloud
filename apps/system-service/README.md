...
- 页面提供工作台总览、图片转粒子、JSON 对比、ENV 对比、周报生成、BullMQ 面板和配置版本预览七个 hash 入口
...
- 周报生成通过 GitHub Commits API 读取配置白名单中的 GitHub 仓库，使用 `GITHUB_TAG_FILE_TOKEN` 系统环境变量；页面通过 Job 状态轮询显示后端真实进度，单个项目失败时跳过并返回 `projectErrors`，其余项目继续生成可复制/下载的 Markdown。启用飞书发布后，`last-week` 周报会按月份选择配置的 Sheet，仅匹配 A 列日期范围和 B 列姓名，并只更新该行 C-G 五个工作日单元格；相同内容跳过，发布失败仍保留 Markdown。
