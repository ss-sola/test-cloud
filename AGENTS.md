...
- 子项目根目录的 `AGENTS.md` 只补充该子项目的专属规则；与本文件冲突时，以更具体的子项目规则为准。
- 涉及共享能力时，同时遵守 `apps/common-service/AGENTS.md`。
...
- 新增非 trivial 业务规则优先补行为测试；可稳定复现的缺陷必须补回归测试；优先覆盖公开方法、核心筛选、配置合并、缓存策略、身份同步和导入转换等高价值规则,不要添加一堆无意义的字符串格式验证。
- 任务完成后在项目根目录执行：
  - `pnpm run lint`
  - `pnpm run test`
  - `pnpm run tsgo`
